...
The Isaac API offers a `Mod.SaveData` method to store data into a "save#.dat" file. Since this method takes a string, you must first convert all of your data to a string. The naive way to accomplish to have every variable in the mod live on a shared table, and then use the output of `json.encode` to store it. And then we can use the output of `json.decode` to restore it. Easy!
